...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, list_IDs, labels, dim, mp_pooler, augmentation=True, batch_size=32, nfft=512, spec_len=250,dim_h=112,dim_w=112,
    ...
    def __data_generation_mp(self, list_IDs_temp, indexes):
        X = [self.mp_pooler.apply_async(ut.load_data,
                                        args=(ID, self.wdim,self.hdim,self.win_length, self.sr, self.hop_length,
                                        self.nfft, self.spec_len)) for ID in list_IDs_temp]
        
        X = np.expand_dims(np.array([p.get() for p in X]), -1)
        X=X.reshape(self.batch_size,self.frame_num,self.hdim,self.wdim,3)
