...
from keras.layers import Activation, Conv1D, Conv2D, Input, Lambda,Conv3D
from keras.layers import BatchNormalization, Flatten, Dense, Reshape
from keras.layers import MaxPooling2D, AveragePooling2D, GlobalAveragePooling2D
from keras.layers import MaxPooling3D, AveragePooling3D, GlobalAveragePooling3D

weight_decay = 1e-4


def identity_block_3D(input_tensor, kernel_size, filters, stage, block, trainable=True):
...
def conv_block_3D(input_tensor, kernel_size, filters, stage, block, strides=(2, 2,1), trainable=True):
...
def resnet_3D_v1(input_dim, mode='train'):
    ...
    # ===============================================
    x4 = conv_block_3D(x3, 3, [128, 128, 256], stage=4, block='a', trainable=True)
    x4 = identity_block_3D(x4, 3, [128, 128, 256], stage=4, block='b', trainable=True)
    x4 = identity_block_3D(x4, 3, [128, 128, 256], stage=4, block='c', trainable=True)
    # ===============================================
    #            Convolution Section 5
    # ===============================================
    x5 = conv_block_3D(x4, 3, [256, 256, 512], stage=5, block='a', trainable=True)
    x5 = identity_block_3D(x5, 3, [256, 256, 512], stage=5, block='b', trainable=True)
    x5 = identity_block_3D(x5, 3, [256, 256, 512], stage=5, block='c', trainable=True)
