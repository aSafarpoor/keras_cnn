...
def vggvox_resnet2d_icassp(input_dim=(250,112,112,3), num_class=8631, mode='train', args=None):
    ...
    # input_dim=(1,80,30,30,3)
    mgpu = len(keras.backend.tensorflow_backend._get_available_gpus())

    if net == 'resnet34s':
        inputs, x = backbone.resnet_3D_v1(input_dim=input_dim, mode=mode)
    else:
        inputs, x = backbone.resnet_3D_v2(input_dim=input_dim, mode=mode)
    # ===============================================
    #            Fully Connected Block 1
    # ===============================================
    x_fc = keras.layers.Conv3D(bottleneck_dim, (1, 1,1),
    ...
    if aggregation == 'avg':
        if mode == 'train':
            x = keras.layers.AveragePooling3D((1, 1,1), strides=(1,1, 1), name='avg_pool')(x)
        ...
        x_k_center = keras.layers.Conv3D(vlad_clusters, (1,1, 1),
                                         strides=(1, 1,1),
                                         kernel_initializer='orthogonal',
                                         use_bias=True, trainable=True,
                                         kernel_regularizer=keras.regularizers.l2(weight_decay),
        ...
        x_k_center = keras.layers.Conv3D(vlad_clusters+ghost_clusters, (1,1,1),
                                         strides=(1,1, 1),
                                         kernel_initializer='orthogonal',
                                         use_bias=True, trainable=True,
                                         kernel_regularizer=keras.regularizers.l2(weight_decay),
